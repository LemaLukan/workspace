...
public class A5
{
	...
	private static Result assignment(int index)
	{
		...
		i++;
		if (tokens.get(i).sym  == 2 && tokens.get(i+1).sym == 14)
		{
			store.end = i + 1;
			store.match = true;
			return store;
	...
	public static void main(String[] args) throws Exception {
		...
		Symbol token; 
		int i = 0;
		while ((token=scanner.next_token()).sym != A5Sym.EOF) {
			tokens.add(token);
		}
		tokens.add(token);   // add EOF as the last token in the array
		boolean legal= program(0); //&& token.sym==A5Sym.EOF;
